...
**Plan d'exécution découpé en 7 sessions : [`.claude/tasks/plugin-split.md`](tasks/plugin-split.md)** — préconditions, étapes, commandes de vérification et critère de sortie par session, plus 6 invariants et un journal de reprise. **S1, S2 et S3 faites le 2026-08-02** ; prochaine session : **S4 (les quatre `release.sh` : `edifice`/`gtm`/`pm` en 0.1.0, `hal` en 0.12.0, puis la note de migration à coller)**, sur la branche `refactor/plugin-split` — suivi configuré, `git push` suffit. **Précondition dure de S4 : working tree strictement propre**, `release.sh` refuse de tourner autrement et fait `git add -A`. La note de migration 0.12.0 est déjà rédigée, dans le plan sous S4.5, prête à coller. Aucune CI ne tourne dessus — `ci.yml` ne se déclenche que sur `pull_request` et sur `push` vers `main` — et **la PR ne s'ouvre qu'en S5**, quand le dépôt redevient cohérent : l'état intermédiaire est fonctionnellement cassé, mais toujours valide pour `check_version_sync.sh`.

Les quatre plugins existent désormais côte à côte : `hal` 0.11.6 réduit à ses quatre fichiers (`plugin.json`, `.mcp.json`, `README.md`, `CHANGELOG.md`), `edifice` / `gtm` / `pm` seedés en **0.0.0** — `release.sh` les promeut en 0.1.0 en S4. Le découpage des fichiers est terminé ; ne restent que les versions (S4) et la documentation du dépôt (S5).
